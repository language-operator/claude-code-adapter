...
Cut a new release of `claude-code-adapter`. The bump type is: **$ARGUMENTS**
...
- `.github/workflows/build-image.yaml` — builds and pushes the adapter image to `ghcr.io/language-operator/claude-code-adapter`, tagged `X.Y.Z`, `X.Y`, `X`, and `sha-<commit>` (the leading `v` is stripped by `docker/metadata-action`).
...
- The resulting artifacts: `ghcr.io/language-operator/claude-code-adapter:X.Y.Z` and `oci://ghcr.io/language-operator/charts/claude-code:X.Y.Z`.
